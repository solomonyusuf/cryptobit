...
const categories = ref(["All", "Bookmark", "Recently Listed", "Launchable", "Meme", "AI & Big Data", "DeFi", "DePIN", "Layer 1","Layer 2", "RWA", "Gaming", "Forex"]);

const showLaunchable = ref(false);

const toggleLaunchable = () => {
  showLaunchable.value = !showLaunchable.value;
  console.log(showLaunchable.value)
};
...
const filteredItems = computed(() => {
  if (selectedCategory.value === 'All') {
    if(searchQuery.value !== '')
      return stocks.value.filter(x => x.asset.title.toLowerCase().includes(searchQuery.value.toLowerCase()));
    else return stocks.value;
  }
  if(searchQuery.value === '')
    return stocks.value.filter(item => item.category.toLowerCase().includes(selectedCategory.value.toLowerCase()));
  else
    return stocks.value.filter(item => item.category.toLowerCase().includes(selectedCategory.value.toLowerCase()))
...
const assetCount = computed(() => filteredItems.value.length ?? 0);
const totalPages = computed(() => Math.ceil(assetCount.value / ItemsPerPage.value));

const startItem = computed(() => (currentPage.value - 1) * ItemsPerPage.value + 1);
const endItem = computed(() => Math.min(currentPage.value * ItemsPerPage.value, assetCount.value));
...
// **Handles page change**
const changePage = (page, emit) => {
  if (page > 0 && page <= totalPages.value) {
    currentPage.value = page;
    if (emit) emit("update:currentPage", page);
  }
};
...
const handleBookmark = (index) => {
  let selectedItem = paginatedItems.value[index];

  if (!selectedItem) return;

  let categories = selectedItem.category.split(",").map(cat => cat.trim());

  if (categories.includes("Bookmark")) {
    categories = categories.filter(cat => cat !== "Bookmark");
  } else {
    categories.push("Bookmark");
  }

  selectedItem.category = categories.join(", ");

 paginatedItems.value.sort((a, b) => {
    const aHasBookmark = a.category.includes("Bookmark") ? -1 : 1;
    const bHasBookmark = b.category.includes("Bookmark") ? -1 : 1;
    return aHasBookmark - bHasBookmark;
  });
};
...
const parseCurrency = (value) => {
  ...
  let multiplier = 1;
  if (value.includes("K")) multiplier = 1_000;
  if (value.includes("M")) multiplier = 1_000_000;
  if (value.includes("B")) multiplier = 1_000_000_000;
  if (value.includes("T")) multiplier = 1_000_000_000_000;

  return parseFloat(value.replace(/[$KMBT]/g, "")) * multiplier;
};

const SortItems = (order = "asc") => {
  if (paginatedItems.value.length < 4) return; // Ensure at least 3 items exist

  paginatedItems.value.sort((a, b) => {
    const aValue = parseCurrency(a.category.split(",")[3] || "0");
    const bValue = parseCurrency(b.category.split(",")[3] || "0");
    ...
    <div class="row">
      <header style="border-bottom: 0.2px solid #5c6b7b" class="sc-6q7ny7-0  cChuYf">
          ...
          <div class="sc-6q7ny7-1 iFqZxr">
              <nav aria-label="Main" data-orientation="horizontal" dir="ltr" class="sc-48njmk-0 gcJXqa sc-6q7ny7-2 eizhqM mt-2">
                  ...
                  <div class="col d-flex flex-column gap-2">
                    <div class="d-flex align-items-center gap-2">
                      <label style="color:#969aa0;" class="form-check-label">
                        Show launchable markets
                      </label>
                      <div class="form-check form-switch mt-3">
                        <input 
                          @input="toggleLaunchable" 
                      ...
                      <div style="font-size: 9px;" id="group" role="group">
                        <button 
                          type="button" 
                        ...
                        <button 
                          ...
                          class="btn btn-sm border-0"
                          style="border-radius: 20px;"
                          :class="getButtonClasses('Recently Listed')"
                          @click="onFilter('Recently Listed')"
                        >
                          Recently Listed
                        </button>
                        ...
                        <button 
                          ...
                          class="btn btn-sm border-0"
                          style="border-radius: 20px;"
                          :class="getButtonClasses('Launchable')"
                          @click="onFilter('Launchable')"
                          
                        >
                          Launchable &nbsp;
                          <span  class="badge rounded-pill bg-label-primary px-2">New</span>  
                        </button>
                        ...
                        <button 
                          ...
                          class="btn btn-sm border-0"
                          style="border-radius: 20px;"
                          :class="getButtonClasses('AI & Big Data')"
                          @click="onFilter('AI & Big Data')"
                        >
                          AI & Big Data
                        </button>
                        ...
                        <button 
                          ...
                          class="btn btn-sm border-0"
                          style="border-radius: 20px;"
                          :class="getButtonClasses('DePIN')"
                          @click="onFilter('DePIN')"
                        >
                          DePIN
                        </button>
                        ...
                        <button 
                          ...
                          class="btn btn-sm border-0"
                          style="border-radius: 20px;"
                          :class="getButtonClasses('Layer 1')"
                          @click="onFilter('Layer 1')"
                        >
                          Layer 1
                        </button>
                        ...
                        <button 
                          ...
                          class="btn btn-sm border-0"
                          style="border-radius: 20px;"
                          :class="getButtonClasses('Layer 2')"
                          @click="onFilter('Layer 2')"
                        >
                          Layer 2
                        </button>
                        ...
                        <button 
                          ...
                          class="btn btn-sm border-0"
                          style="border-radius: 20px;"
                          :class="getButtonClasses('Gaming')"
                          @click="onFilter('Gaming')"
                        >
                        Gaming
                        </button>
                        ...
                        <button 
                          ...
                          class="btn btn-sm border-0"
                          style="border-radius: 20px;"
                          :class="getButtonClasses('Forex')"
                          @click="onFilter('Forex')"
                        >
                        Forex
                        </button>
                      </div>
            ...
            <div class="mt-2 table-responsive text-nowrap " style="max-height: 500px; overflow-y: auto;">
                ...
                <thead>
                    ...
                    <th class="text-capitalize border-0" style="background:#212124;">24h Volume 
                      <button @click="SortItems" style="background:transparent;border:0;">
                      <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 320 512" fill="#576372">
                        <path d="M279 224H41c-21.4 0-32.1-25.9-17-41L143 64c9.4-9.4 24.6-9.4 33.9 0L296 183c15.1 15.1 4.4 41-17 41zM41 288h238c21.4 0 32.1 25.9 17 41l-119 119c-9.4 9.4-24.6 9.4-33.9 0L24 329c-15.1-15.1-4.4-41 17-41z"/>
                      </svg>
                     </button>
                    </th>
                    <th class="text-capitalize border-0" style="background:#212124;">24h Spot Volume</th>
                    <th class="text-capitalize border-0" style="background:#212124;">Market cap</th>
                    <th class="text-capitalize border-0" style="background:#212124;">Trades</th>
                    <th class="text-capitalize border-0" style="background:#212124;">Open intrest</th>
                    <th class="text-capitalize border-0" style="background:#212124;">1h funding</th>
                    </tr>
                </thead>
...
.lay{
  border-top: 0.2px solid #494950; border-bottom: 0.2px solid #494950;
}
...
.form-range:focus::-webkit-slider-thumb {
  -webkit-box-shadow: 0 0 0 1px #f5f5f9, 0 0.125rem 0.25rem 0 rgba(105, 108, 255, 0.4);
...
.nav-link:focus-visible {
  outline: 0;
  -webkit-box-shadow: 0 0 0 0.15rem rgba(34, 48, 62, 0.75);
...
.placeholder-wave {
  -webkit-mask-image: linear-gradient(130deg, #22303e 55%, rgba(0, 0, 0, 0.8) 75%, #22303e 95%);
...
.border-label-dark {
  border: 3px solid #aaabb3 !important;
}
...
.border-label-gray {
  border: 3px solid rgba(215, 217, 220, 0.8) !important;
}
...
.btn-outline-secondary:hover {
  ...
  background-color: #788393 !important;
  border-color: #788393 !important;
  -webkit-box-shadow: 0 0.125rem 0.25rem 0 rgba(133, 146, 163, 0.4) !important;
...
.btn-success {
  ...
  border-color: #71dd37;
  -webkit-box-shadow: 0 0.125rem 0.25rem 0 rgba(113, 221, 55, 0.4);
...
.btn-outline-success:hover {
  ...
  background-color: #66c732 !important;
  border-color: #66c732 !important;
  -webkit-box-shadow: 0 0.125rem 0.25rem 0 rgba(113, 221, 55, 0.4) !important;
...
.btn-info {
  ...
  border-color: #03c3ec;
  -webkit-box-shadow: 0 0.125rem 0.25rem 0 rgba(3, 195, 236, 0.4);
...
.btn-outline-info:hover {
  ...
  background-color: #03b0d4 !important;
  border-color: #03b0d4 !important;
  -webkit-box-shadow: 0 0.125rem 0.25rem 0 rgba(3, 195, 236, 0.4) !important;
...
.btn-warning {
  ...
  border-color: #ffab00;
  -webkit-box-shadow: 0 0.125rem 0.25rem 0 rgba(255, 171, 0, 0.4);
...
.btn-outline-warning:hover {
  ...
  background-color: #e69a00 !important;
  border-color: #e69a00 !important;
  -webkit-box-shadow: 0 0.125rem 0.25rem 0 rgba(255, 171, 0, 0.4) !important;
...
.btn-danger {
  ...
  border-color: #ff3e1d;
  -webkit-box-shadow: 0 0.125rem 0.25rem 0 rgba(255, 62, 29, 0.4);
...
.btn-outline-danger:hover {
  ...
  background-color: #e6381a !important;
  border-color: #e6381a !important;
  -webkit-box-shadow: 0 0.125rem 0.25rem 0 rgba(255, 62, 29, 0.4) !important;
...
.btn-outline-light:hover {
  ...
  -webkit-box-shadow: 0 0.125rem 0.25rem 0 rgba(219, 222, 224, 0.4) !important;
...
.btn-dark {
  ...
  border-color: #2b2c40;
  -webkit-box-shadow: 0 0.125rem 0.25rem 0 rgba(43, 44, 64, 0.4);
...
.btn-outline-dark:hover {
  ...
  background-color: #27283a !important;
  border-color: #27283a !important;
  -webkit-box-shadow: 0 0.125rem 0.25rem 0 rgba(43, 44, 64, 0.4) !important;
...
.btn-gray {
  ...
  -webkit-box-shadow: 0 0.125rem 0.25rem 0 rgba(34, 48, 62, 0.4);
...
.btn-outline-gray:hover {
  ...
  background-color: rgba(26, 36, 47, 0.55) !important;
  border-color: rgba(26, 36, 47, 0.55) !important;
  -webkit-box-shadow: 0 0.125rem 0.25rem 0 rgba(34, 48, 62, 0.4) !important;
...
.alert-secondary .alert-icon {
  background-color: #8592a3;
  -webkit-box-shadow: 0 0 0 0.125rem rgba(133, 146, 163, 0.16);
...
.alert-success .alert-icon {
  background-color: #71dd37;
  -webkit-box-shadow: 0 0 0 0.125rem rgba(113, 221, 55, 0.16);
...
.alert-info .alert-icon {
  background-color: #03c3ec;
  -webkit-box-shadow: 0 0 0 0.125rem rgba(3, 195, 236, 0.16);
...
.alert-warning .alert-icon {
  background-color: #ffab00;
  -webkit-box-shadow: 0 0 0 0.125rem rgba(255, 171, 0, 0.16);
...
.alert-danger .alert-icon {
  background-color: #ff3e1d;
  -webkit-box-shadow: 0 0 0 0.125rem rgba(255, 62, 29, 0.16);
...
.alert-dark .alert-icon {
  background-color: #2b2c40;
  -webkit-box-shadow: 0 0 0 0.125rem rgba(43, 44, 64, 0.16);
...
.alert-gray .alert-icon {
  background-color: rgba(34, 48, 62, 0.5);
  -webkit-box-shadow: 0 0 0 0.125rem rgba(34, 48, 62, 0.16);
...
.form-select {
  background-clip: padding-box;
  padding: calc(0.543rem - var(--bs-border-width)) calc(0.9375rem - var(--bs-border-width));
  -webkit-padding-end: calc(2.8125rem - var(--bs-border-width));
...
.form-select.form-select-lg {
  ...
  padding: calc(0.7075rem - var(--bs-border-width)) calc(1.3125rem - var(--bs-border-width));
  -webkit-padding-end: calc(3.9375rem - var(--bs-border-width));
...
.form-select.form-select-sm {
  ...
  padding: calc(0.3165rem - var(--bs-border-width)) calc(0.8125rem - var(--bs-border-width));
  -webkit-padding-end: calc(2.4375rem - var(--bs-border-width));
...
.form-check-input:checked {
  -webkit-box-shadow: 0 0.125rem 0.375rem 0 rgba(34, 48, 62, 0.08);
...
.form-switch .form-check-input {
  ...
  border: none;
  -webkit-box-shadow: 0 0 0.25rem 0 rgba(0, 0, 0, 0.16) inset;
...
.input-group:focus-within .form-select:first-child {
  -webkit-padding-start: calc(0.9375rem - 2px);
...
form .form-label.invalid, form .form-label.is-invalid {
  border-width: 2px;
  border-color: #ff3e1d;
  -webkit-box-shadow: 0 0 0 2px rgba(255, 62, 29, 0.4) !important;
...
.modal .btn-close {
  ...
  opacity: 1;
  padding: 0.563rem;
  -webkit-box-shadow: 0 0.0625rem 0.3175rem 0 rgba(34, 48, 62, 0.06);
  ...
  -webkit-transform: translate(23px, -25px);
      -ms-transform: translate(23px, -25px);
...
.progress-bar.bg-secondary {
  -webkit-box-shadow: 0 2px 4px 0 rgba(133, 146, 163, 0.4);
          box-shadow: 0 2px 4px 0 rgba(133, 146, 163, 0.4);
}

.progress-bar.bg-success {
  -webkit-box-shadow: 0 2px 4px 0 rgba(113, 221, 55, 0.4);
          box-shadow: 0 2px 4px 0 rgba(113, 221, 55, 0.4);
}

.progress-bar.bg-info {
  -webkit-box-shadow: 0 2px 4px 0 rgba(3, 195, 236, 0.4);
          box-shadow: 0 2px 4px 0 rgba(3, 195, 236, 0.4);
}

.progress-bar.bg-warning {
  -webkit-box-shadow: 0 2px 4px 0 rgba(255, 171, 0, 0.4);
          box-shadow: 0 2px 4px 0 rgba(255, 171, 0, 0.4);
}

.progress-bar.bg-danger {
  -webkit-box-shadow: 0 2px 4px 0 rgba(255, 62, 29, 0.4);
          box-shadow: 0 2px 4px 0 rgba(255, 62, 29, 0.4);
}

.progress-bar.bg-light {
  -webkit-box-shadow: 0 2px 4px 0 rgba(219, 222, 224, 0.4);
          box-shadow: 0 2px 4px 0 rgba(219, 222, 224, 0.4);
}

.progress-bar.bg-dark {
  -webkit-box-shadow: 0 2px 4px 0 rgba(43, 44, 64, 0.4);
          box-shadow: 0 2px 4px 0 rgba(43, 44, 64, 0.4);
}

.progress-bar.bg-gray {
  -webkit-box-shadow: 0 2px 4px 0 rgba(34, 48, 62, 0.4);
...
.accordion .accordion-item.active .accordion-button:not(.collapsed) {
  -webkit-box-shadow: none;
          box-shadow: none;
}
...
.bg-secondary.toast, .bg-secondary.bs-toast {
  ...
  -webkit-box-shadow: 0 0.25rem 1rem rgba(133, 146, 163, 0.4);
...
.bg-secondary.toast .toast-header .btn-close, .bg-secondary.bs-toast .toast-header .btn-close {
  ...
  -webkit-box-shadow: 0 0.1875rem 0.375rem 0 rgba(133, 146, 163, 0.4) !important;
...
.bg-success.toast, .bg-success.bs-toast {
  ...
  -webkit-box-shadow: 0 0.25rem 1rem rgba(113, 221, 55, 0.4);
...
.bg-success.toast .toast-header .btn-close, .bg-success.bs-toast .toast-header .btn-close {
  background-color: #71dd37 !important;
  background-image: url("data:image/svg+xml,%3Csvg width='150px' height='151px' viewBox='0 0 150 151' version='1.1' xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink'%3E%3Cdefs%3E%3Cpolygon id='path-1' points='131.251657 0 74.9933705 56.25 18.7483426 0 0 18.75 56.2450278 75 0 131.25 18.7483426 150 74.9933705 93.75 131.251657 150 150 131.25 93.7549722 75 150 18.75'%3E%3C/polygon%3E%3C/defs%3E%3Cg id='🎨-%5BSetup%5D:-Colors-&amp;-Shadows' stroke='none' stroke-width='1' fill='none' fill-rule='evenodd'%3E%3Cg id='Artboard' transform='translate(-225.000000, -250.000000)'%3E%3Cg id='Icon-Color' transform='translate(225.000000, 250.500000)'%3E%3Cuse fill='%23fff' xlink:href='%23path-1'%3E%3C/use%3E%3Cuse fill-opacity='1' fill='%23fff' xlink:href='%23path-1'%3E%3C/use%3E%3C/g%3E%3C/g%3E%3C/g%3E%3C/svg%3E");
  -webkit-box-shadow: 0 0.1875rem 0.375rem 0 rgba(113, 221, 55, 0.4) !important;
...
.bg-info.toast, .bg-info.bs-toast {
  ...
  -webkit-box-shadow: 0 0.25rem 1rem rgba(3, 195, 236, 0.4);
...
.bg-info.toast .toast-header .btn-close, .bg-info.bs-toast .toast-header .btn-close {
  background-color: #03c3ec !important;
  background-image: url("data:image/svg+xml,%3Csvg width='150px' height='151px' viewBox='0 0 150 151' version='1.1' xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink'%3E%3Cdefs%3E%3Cpolygon id='path-1' points='131.251657 0 74.9933705 56.25 18.7483426 0 0 18.75 56.2450278 75 0 131.25 18.7483426 150 74.9933705 93.75 131.251657 150 150 131.25 93.7549722 75 150 18.75'%3E%3C/polygon%3E%3C/defs%3E%3Cg id='🎨-%5BSetup%5D:-Colors-&amp;-Shadows' stroke='none' stroke-width='1' fill='none' fill-rule='evenodd'%3E%3Cg id='Artboard' transform='translate(-225.000000, -250.000000)'%3E%3Cg id='Icon-Color' transform='translate(225.000000, 250.500000)'%3E%3Cuse fill='%23fff' xlink:href='%23path-1'%3E%3C/use%3E%3Cuse fill-opacity='1' fill='%23fff' xlink:href='%23path-1'%3E%3C/use%3E%3C/g%3E%3C/g%3E%3C/g%3E%3C/svg%3E");
  -webkit-box-shadow: 0 0.1875rem 0.375rem 0 rgba(3, 195, 236, 0.4) !important;
...
.bg-warning.toast, .bg-warning.bs-toast {
  ...
  -webkit-box-shadow: 0 0.25rem 1rem rgba(255, 171, 0, 0.4);
...
.bg-warning.toast .toast-header .btn-close, .bg-warning.bs-toast .toast-header .btn-close {
  background-color: #ffab00 !important;
  background-image: url("data:image/svg+xml,%3Csvg width='150px' height='151px' viewBox='0 0 150 151' version='1.1' xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink'%3E%3Cdefs%3E%3Cpolygon id='path-1' points='131.251657 0 74.9933705 56.25 18.7483426 0 0 18.75 56.2450278 75 0 131.25 18.7483426 150 74.9933705 93.75 131.251657 150 150 131.25 93.7549722 75 150 18.75'%3E%3C/polygon%3E%3C/defs%3E%3Cg id='🎨-%5BSetup%5D:-Colors-&amp;-Shadows' stroke='none' stroke-width='1' fill='none' fill-rule='evenodd'%3E%3Cg id='Artboard' transform='translate(-225.000000, -250.000000)'%3E%3Cg id='Icon-Color' transform='translate(225.000000, 250.500000)'%3E%3Cuse fill='%23fff' xlink:href='%23path-1'%3E%3C/use%3E%3Cuse fill-opacity='1' fill='%23fff' xlink:href='%23path-1'%3E%3C/use%3E%3C/g%3E%3C/g%3E%3C/g%3E%3C/svg%3E");
  -webkit-box-shadow: 0 0.1875rem 0.375rem 0 rgba(255, 171, 0, 0.4) !important;
...
.bg-danger.toast, .bg-danger.bs-toast {
  ...
  -webkit-box-shadow: 0 0.25rem 1rem rgba(255, 62, 29, 0.4);
...
.bg-danger.toast .toast-header .btn-close, .bg-danger.bs-toast .toast-header .btn-close {
  background-color: #ff3e1d !important;
  background-image: url("data:image/svg+xml,%3Csvg width='150px' height='151px' viewBox='0 0 150 151' version='1.1' xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink'%3E%3Cdefs%3E%3Cpolygon id='path-1' points='131.251657 0 74.9933705 56.25 18.7483426 0 0 18.75 56.2450278 75 0 131.25 18.7483426 150 74.9933705 93.75 131.251657 150 150 131.25 93.7549722 75 150 18.75'%3E%3C/polygon%3E%3C/defs%3E%3Cg id='🎨-%5BSetup%5D:-Colors-&amp;-Shadows' stroke='none' stroke-width='1' fill='none' fill-rule='evenodd'%3E%3Cg id='Artboard' transform='translate(-225.000000, -250.000000)'%3E%3Cg id='Icon-Color' transform='translate(225.000000, 250.500000)'%3E%3Cuse fill='%23fff' xlink:href='%23path-1'%3E%3C/use%3E%3Cuse fill-opacity='1' fill='%23fff' xlink:href='%23path-1'%3E%3C/use%3E%3C/g%3E%3C/g%3E%3C/g%3E%3C/svg%3E");
  -webkit-box-shadow: 0 0.1875rem 0.375rem 0 rgba(255, 62, 29, 0.4) !important;
...
.bg-light.toast, .bg-light.bs-toast {
  ...
  -webkit-box-shadow: 0 0.25rem 1rem rgba(219, 222, 224, 0.4);
...
.bg-light.toast .toast-header .btn-close, .bg-light.bs-toast .toast-header .btn-close {
  background-color: #dbdee0 !important;
  background-image: url("data:image/svg+xml,%3Csvg width='150px' height='151px' viewBox='0 0 150 151' version='1.1' xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink'%3E%3Cdefs%3E%3Cpolygon id='path-1' points='131.251657 0 74.9933705 56.25 18.7483426 0 0 18.75 56.2450278 75 0 131.25 18.7483426 150 74.9933705 93.75 131.251657 150 150 131.25 93.7549722 75 150 18.75'%3E%3C/polygon%3E%3C/defs%3E%3Cg id='🎨-%5BSetup%5D:-Colors-&amp;-Shadows' stroke='none' stroke-width='1' fill='none' fill-rule='evenodd'%3E%3Cg id='Artboard' transform='translate(-225.000000, -250.000000)'%3E%3Cg id='Icon-Color' transform='translate(225.000000, 250.500000)'%3E%3Cuse fill='%23fff' xlink:href='%23path-1'%3E%3C/use%3E%3Cuse fill-opacity='1' fill='%23fff' xlink:href='%23path-1'%3E%3C/use%3E%3C/g%3E%3C/g%3E%3C/g%3E%3C/svg%3E");
  -webkit-box-shadow: 0 0.1875rem 0.375rem 0 rgba(219, 222, 224, 0.4) !important;
...
.bg-dark.toast, .bg-dark.bs-toast {
  ...
  -webkit-box-shadow: 0 0.25rem 1rem rgba(43, 44, 64, 0.4);
...
.bg-dark.toast .toast-header .btn-close, .bg-dark.bs-toast .toast-header .btn-close {
  background-color: #2b2c40 !important;
  background-image: url("data:image/svg+xml,%3Csvg width='150px' height='151px' viewBox='0 0 150 151' version='1.1' xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink'%3E%3Cdefs%3E%3Cpolygon id='path-1' points='131.251657 0 74.9933705 56.25 18.7483426 0 0 18.75 56.2450278 75 0 131.25 18.7483426 150 74.9933705 93.75 131.251657 150 150 131.25 93.7549722 75 150 18.75'%3E%3C/polygon%3E%3C/defs%3E%3Cg id='🎨-%5BSetup%5D:-Colors-&amp;-Shadows' stroke='none' stroke-width='1' fill='none' fill-rule='evenodd'%3E%3Cg id='Artboard' transform='translate(-225.000000, -250.000000)'%3E%3Cg id='Icon-Color' transform='translate(225.000000, 250.500000)'%3E%3Cuse fill='%23fff' xlink:href='%23path-1'%3E%3C/use%3E%3Cuse fill-opacity='1' fill='%23fff' xlink:href='%23path-1'%3E%3C/use%3E%3C/g%3E%3C/g%3E%3C/g%3E%3C/svg%3E");
  -webkit-box-shadow: 0 0.1875rem 0.375rem 0 rgba(43, 44, 64, 0.4) !important;
...
.bg-gray.toast, .bg-gray.bs-toast {
  ...
  -webkit-box-shadow: 0 0.25rem 1rem rgba(34, 48, 62, 0.4);
...
.bg-gray.toast .toast-header .btn-close, .bg-gray.bs-toast .toast-header .btn-close {
  background-color: rgba(34, 48, 62, 0.5) !important;
  background-image: url("data:image/svg+xml,%3Csvg width='150px' height='151px' viewBox='0 0 150 151' version='1.1' xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink'%3E%3Cdefs%3E%3Cpolygon id='path-1' points='131.251657 0 74.9933705 56.25 18.7483426 0 0 18.75 56.2450278 75 0 131.25 18.7483426 150 74.9933705 93.75 131.251657 150 150 131.25 93.7549722 75 150 18.75'%3E%3C/polygon%3E%3C/defs%3E%3Cg id='🎨-%5BSetup%5D:-Colors-&amp;-Shadows' stroke='none' stroke-width='1' fill='none' fill-rule='evenodd'%3E%3Cg id='Artboard' transform='translate(-225.000000, -250.000000)'%3E%3Cg id='Icon-Color' transform='translate(225.000000, 250.500000)'%3E%3Cuse fill='%23fff' xlink:href='%23path-1'%3E%3C/use%3E%3Cuse fill-opacity='1' fill='%23fff' xlink:href='%23path-1'%3E%3C/use%3E%3C/g%3E%3C/g%3E%3C/g%3E%3C/svg%3E");
  -webkit-box-shadow: 0 0.1875rem 0.375rem 0 rgba(34, 48, 62, 0.4) !important;
...
.shadow {
  -webkit-box-shadow: 0 0.1875rem 0.5rem 0 rgba(34, 48, 62, 0.1) !important;
          box-shadow: 0 0.1875rem 0.5rem 0 rgba(34, 48, 62, 0.1) !important;
}

.shadow-sm {
  -webkit-box-shadow: 0 0.125rem 0.375rem 0 rgba(34, 48, 62, 0.08) !important;
          box-shadow: 0 0.125rem 0.375rem 0 rgba(34, 48, 62, 0.08) !important;
}

.shadow-lg {
  -webkit-box-shadow: 0 0.25rem 0.75rem 0 rgba(34, 48, 62, 0.14) !important;
          box-shadow: 0 0.25rem 0.75rem 0 rgba(34, 48, 62, 0.14) !important;
}

.shadow-xl {
  -webkit-box-shadow: 0 0.3125rem 1.375rem 0 rgba(34, 48, 62, 0.18) !important;
...
.img-thumbnail-shadow:hover, .img-thumbnail-shadow:focus {
  -webkit-box-shadow: 0 5px 20px rgba(34, 48, 62, 0.4);
...
.buy-now .btn-buy-now {
  ...
  right: 1.625rem;
  z-index: 1080;
  -webkit-box-shadow: 0 1px 20px 1px #ff3e1d;
...
.menu:not(.menu-no-animation) .menu-link {
  ...
  -webkit-transition-property: color, background-color, left;
...
.layout-wrapper .layout-navbar .dropdown-menu[data-bs-popper] {
  top: 144%;
}
